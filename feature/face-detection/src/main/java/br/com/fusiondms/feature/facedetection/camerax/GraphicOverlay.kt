...
open class GraphicOverlay(context: Context?, attrs: AttributeSet?) :
    ...
    abstract class Graphic(private val overlay: GraphicOverlay) {
        ...
        fun getDisplayMetrics(type: String): Int {
            return if (type == "width") overlay.context.resources.displayMetrics.widthPixels
            else overlay.context.resources.displayMetrics.heightPixels
        }
        ...
        fun scaleX(horizontal: Float) = overlay.mOffsetX?.let {
            horizontal * it
        }

        fun scaleY(vertical: Float) = overlay.mOffsetY?.let {
            vertical * it
        }
    ...
    private fun initProcessCanvas () {
        processBitmap = Bitmap.createBitmap(1000, 1000, Bitmap.Config.ARGB_8888)
